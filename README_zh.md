...
- **真机构建只实现了 BME280**：合成基准包含光照通道以检验多模态行为，但本仓库没有
  BH1750 驱动：光照通道被标记为无效，MQTT 负载发送 `"light": null` 并附带显式的
  `valid` 映射，而不是一个看起来合理的 `0`。BH1750 属于后续硬件扩展。
...
| **ESP32-S3 真机烧录 / 运行** | `Not measured yet.` |
| **BME280 实机传感器验证** | `Not measured yet.` |
| **Wi-Fi / MQTT 多周期真机运行** | `Not measured yet.` |
| **功耗测量** | `Not measured yet.` —— 未做 INA219 / Joulescope / Power Profiler 测量 |
| BH1750 光照传感器 | **未实现**；该通道被标记为无效并以 `null` 发送 |
