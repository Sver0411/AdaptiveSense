...
3. **真机模态覆盖温度、湿度与光照**：板上装有 SHT30 与 BH1750；`pressure`
   未在真机验证（板上没有 BME280），驱动仅在主机侧测试。仿真仍独立建模全部
   3 个合成通道，与板上有什么无关。
4. **没有真机能量测量**：所有成本量都是计数与无量纲代理量。
5. **运行期 I²C 总线被拉死没有自动恢复**：`sensor_bus.c` 的总线恢复流程只在
   创建总线时执行（开机），不在运行中；若有从机在节点运行期间持续拉低
   SDA/SCL，需要一套尚未设计、尚未测试的重探流程。曾观察到一次并手工解除；
   见 `docs/hardware.md`。
6. **变化感知策略无法在变化被采到之前做出反应**。若短事件期间没有采样点，
   任何仅依赖采样观测的策略都无法检出（场景 E）。
7. **参数是人工选定的，且已知并非联合最优。**
   `event_min_duration_s`（10 s）并未显著小于 `baseline_tau_s`（60 s），
   导致在 60 s 间隔下观测到的阶跃无法在偏差衰减前满足去抖（场景 C）。
   这些取值**按原配置保留，没有为了改善已发布数字而调参**。
8. **没有与更先进的自适应采样算法对比**（变点检测、贝叶斯或信息论方法、学习式预测器）。
9. **"检出"意味着"分数越过阈值并保持足够久"**，而不是"变点被正确定位"：
   检测器是阈值 + 去抖规则。
10. **`publish_call_ok` 不等于投递确认**：它只表示 MQTT 客户端接受了请求（QoS 0）。
